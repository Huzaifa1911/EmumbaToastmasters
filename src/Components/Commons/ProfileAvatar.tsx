...
interface IProfileAvatarProps extends Omit<AvatarImageProps, 'source' | 'onError'> {
  uri: string;
}
...
const ProfileAvatar = ({size = 50, uri, ...rest}: IProfileAvatarProps) => {
  ...
  return <Avatar.Image size={size} source={{uri: hasError ? DEFAULT_IMAGE : uri}} onError={onError} {...rest} />;
};
