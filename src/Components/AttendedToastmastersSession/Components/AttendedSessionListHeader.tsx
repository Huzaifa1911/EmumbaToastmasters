...
const goToViewAllSessionScreen = () =>
  NavigationService.navigate(SCREENS.VIEW_ALL_ATTENDED_SESSIONS_SCREEN);

const AttendedSessionListHeader = ({
  showViewAllComponent,
}: {
  ...
  return (
    <Spacer top={30} bottom={10}>
      <If condition={showViewAllComponent}>
        <Then>
          <ViewAllTextComponent
            label="Attended Sessions"
            onViewAllPress={goToViewAllSessionScreen}
          />
        </Then>
        <Else>
